...
# các đường dẫn
root_data_dir = f"{get_path('data_dir')}/"
transcript_dir = "processed_transcripts/"
metadata_dir = "metadata.json"
output_dir = f"{get_path('chunks_dir')}/"
# ==========================================================
...
class TranscriptLoader(BaseLoader):
    ...
    def __call__(self, txt_files: List[str], metadata_path: str, workers: int = 2) -> List[dict]:
        ...
        # Load sequentially to avoid multiprocessing deadlock on Windows
...
class Loader:
    ...
    def load_dir(self, root_data_dir: str, transcript_dir: str, metadata_dir: str, output_dir: str, workers: int = 1, target_playlist: str = None):
        all_chunks = []
        try:
        ...
        for playlist_folder in tqdm(playlists, desc="Playlists", unit="playlist"):
            try:
                ...
                # Chunk and save all docs for this playlist at once
                print(f"[{playlist_folder}] Chunking {len(docs)} documents...")
                out = self.chunker(docs, output_path)
