...
class QuizQuestion(BaseModel):
    ...
    options: List[str] = Field(
        description="Mảng chứa đúng 4 lựa chọn (A, B, C, D) dưới dạng chuỗi"
    )
    correct_answer: str = Field(description="Lựa chọn đúng (chỉ nội dung lựa chọn)")
    explanation: str = Field(
        description="Giải thích ngắn gọn bám sát nội dung trong video transcript"
    )
    video_url: str = Field(
        description="URL video tương ứng với kiến thức trong câu hỏi"
    )
    video_title: str = Field(description="Tiêu đề bài giảng chứa kiến thức này")
    timestamp: str = Field(
        description="Thời điểm (format HH:MM:SS) trong video nói về kiến thức này"
    )
