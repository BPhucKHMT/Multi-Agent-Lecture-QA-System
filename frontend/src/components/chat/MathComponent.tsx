  ...
  return (
    <div className="flex flex-col gap-6 py-2">
      ...
      <div className="flex items-center gap-3 border-b border-slate-200/60 pb-4">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-teal-600 text-white shadow-md shadow-teal-600/10">
          <Target className="h-5 w-5" />
        </div>
        <div>
          <h4 className="text-[14px] font-bold uppercase tracking-wider text-teal-600">Mục tiêu giải quyết</h4>
          ...
          data.steps.map((step, index) => (
            ...
            >
              ...
              {index < data.steps.length - 1 && (
                <div className="absolute left-4 top-10 bottom-0 w-0.5 bg-slate-100 group-hover:bg-teal-100 transition-colors" />
              )}

              {/* Step Number Badge */}
              <div className="z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white border-2 border-slate-200 text-sm font-black text-slate-500 shadow-sm transition-colors group-hover:border-teal-600 group-hover:bg-teal-600 group-hover:text-white">
                {index + 1}
              </div>

              {/* Step Content Card */}
              <div className="flex-1 rounded-2xl border border-slate-100 bg-slate-50/30 p-4 transition-all duration-300 hover:border-slate-200 hover:bg-white hover:shadow-[0_4px_20px_rgba(15,23,42,0.04)]">
                {step.title && (
