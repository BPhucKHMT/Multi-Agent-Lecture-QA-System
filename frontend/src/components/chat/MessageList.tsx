...
function LoadingSteps({ status }: { status?: string | null }) {
  ...
  return (
    <div className="min-w-[260px] max-w-[420px] py-1">
      <div className="mb-3 flex items-center gap-2 text-[13px] font-semibold text-teal-700">
      ...
      <div className="mt-4 space-y-1.5">
        <div className="h-1.5 w-full animate-pulse rounded-full bg-slate-200/60" />
        <div className="h-1.5 w-4/5 animate-pulse rounded-full bg-slate-200/60 [animation-delay:120ms]" />
      </div>
    </div>
  );
}
          ...
          >
            ...
            <div className={`group relative w-fit min-w-0 min-h-[44px] max-w-[85%] sm:max-w-[75%] px-6 py-3 transition-all duration-300 flex items-center overflow-hidden ${
              isUser
                ...
                : `${isSmallBubble ? "rounded-[1.75rem]" : "rounded-[1.75rem] rounded-tl-none"} border border-slate-200/80 bg-white/80 text-slate-800 shadow-[0_2px_8px_rgba(15,23,42,0.03)] backdrop-blur-md hover:border-slate-300/80 hover:bg-white hover:shadow-[0_8px_24px_rgba(15,23,42,0.06)]`
            }`}>
              {isUser ? (
